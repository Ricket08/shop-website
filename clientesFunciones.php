...
function traerToken($user_id, $con){

    $sql = $con->prepare("SELECT token_password FROM usuarios WHERE id = ? AND activacion = 1 AND password_request = 1 LIMIT 1");
    $sql->execute([$user_id]);
...
function actualizaPassword($user_id, $password, $con){
    $sql = $con->prepare("UPDATE usuarios SET password = ?, token_password = '',
